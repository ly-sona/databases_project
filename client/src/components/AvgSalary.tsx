...
const columns = [
  ...
  columnHelper.accessor((row: SalaryData) => `${row.FName} ${row.LName}`, {
    id: "name",
    header: "Name",
    cell: (info: { getValue: () => string }) => info.getValue(),
  }),
  columnHelper.accessor("AvgMonthlySalary", {
    header: "Average Monthly Salary",
...
interface AvgSalaryProps {
  compact?: boolean;
  defaultPageSize?: number;
}

export default function AvgSalary({ compact = false, defaultPageSize = 10 }: AvgSalaryProps) {
  const { data, isLoading, error } = useEndpoint<SalaryData[]>("salary", "/avg-salary");
    ...
    <div className={containerClasses}>
      {!compact && (
        <div className="p-6 border-b border-slate-100 bg-gradient-to-r from-sky-50 to-white">
          ...
          <p className="text-slate-500 text-sm">Breakdown of monthly compensation by employee</p>
        </div>
      )}
      
      {compact && (
        <div className="px-2 py-1 border-b border-slate-100">
          <h3 className="text-sm font-medium text-slate-800">Average Monthly Salary per Employee</h3>
        </div>
      )}
