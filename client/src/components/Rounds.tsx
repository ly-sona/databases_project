...
const columnHelper = createColumnHelper<RoundsData>();

const columns = [
  columnHelper.accessor((row: RoundsData) => `${row.FName} ${row.LName}`, {
    id: "name",
    header: "Interviewee",
  ...
  columnHelper.accessor("RoundsPassed", {
    header: "Rounds Passed",
...
export default function Rounds({ compact = false, defaultPageSize = 10 }: RoundsProps) {
  const { data, isLoading, error } = useEndpoint<RoundsData[]>("rounds", "/rounds");
      ...
      {!compact && (
        <div className="p-6 border-b border-slate-100 bg-gradient-to-r from-sky-50 to-white">
          <h2 className="text-2xl font-semibold text-slate-800 mb-1">
            Interview Rounds Passed
          </h2>
          <p className="text-slate-500 text-sm">Rounds passed by interviewee and job</p>
        </div>
      )}
      
      {compact && (
        <div className="px-2 py-1 border-b border-slate-100">
          <h3 className="text-sm font-medium text-slate-800">Interview Rounds Passed</h3>
        </div>
      )}
