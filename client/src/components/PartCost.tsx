...
import type { SortingState, ColumnFiltersState, CellContext } from "@tanstack/react-table";
import { useState } from "react";

interface PartCostData {
  ProductID: number;
  ProdType: string;
  ListPrice: number;
...
const columnHelper = createColumnHelper<PartCostData>();

const columns = [
  columnHelper.accessor("ProductID", {
    header: "Product ID",
    cell: (info: CellContext<PartCostData, number>) => info.getValue(),
  }),
  columnHelper.accessor("ProdType", {
    header: "Type",
    cell: (info: CellContext<PartCostData, string>) => info.getValue(),
  }),
  columnHelper.accessor("ListPrice", {
    header: "List Price",
    cell: (info: CellContext<PartCostData, number>) => `$${Number(info.getValue()).toFixed(2)}`,
  }),
  columnHelper.accessor("TotalPartCost", {
    header: "Total Part Cost",
    cell: (info: CellContext<PartCostData, number>) => `$${Number(info.getValue()).toFixed(2)}`,
  }),
  columnHelper.accessor(
    row => ((row.ListPrice - row.TotalPartCost) / row.ListPrice * 100).toFixed(1),
    {
      id: "profitMargin",
...
interface PartCostProps {
  compact?: boolean;
  defaultPageSize?: number;
}

export default function PartCost({ compact = false, defaultPageSize = 10 }: PartCostProps) {
  const { data, isLoading, error } = useEndpoint<PartCostData[]>("partcost", "/part-cost");
    ...
    <div className={containerClasses}>
      {!compact && (
        <div className="p-6 border-b border-slate-100 bg-gradient-to-r from-sky-50 to-white">
          ...
          <p className="text-slate-500 text-sm">List price, part cost, and margin by product</p>
        </div>
      )}
      
      {compact && (
        <div className="px-2 py-1 border-b border-slate-100">
          <h3 className="text-sm font-medium text-slate-800">Parts Cost per Product</h3>
        </div>
      )}
